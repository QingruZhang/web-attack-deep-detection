...
def split_task():
    ...
    while section+section_size<=len(id_list):
        ...
        for ssub in sub_id:
            pid_group[ssub]=i
        with open('id_group/id_group_%d.json'%i,'w') as f:
            json.dump(sub_id,open('id_group/id_group_%d.json'%i,'w'))
        section+=section_size
        i+=1
    with open('id_group/id_group_%d.json'%i,'w') as tmpfile:
        json.dump(id_list[section:],tmpfile)
    with open('KeyToGroup.json','w') as m:
        json.dump(pid_group,m)

def get_train_group(group):
    train_id=json.load(open('id_group/id_group_%d.json'%group,'w'))
    return train_id

def get_p_dict(table):
    #get raw data from mysql
    sql="select * from table where domain like '%.shafc.edu.cn' ;".replace('table',table)
    # load raw data from local
    data_list=json.load(open('%s_data.json'%table,'r'))
    global p_dict
    remove_list=[
        'http_type',
        'content_length'
    ...
    for key in list(p_dict):
        if len(p_dict[key]["p_states"]) <min_train_num or p_dict[key]['p_type'] in remove_list:
            p_dict.pop(key)
    with open('p_dict.json','w') as f_p:
        json.dump(p_dict,f_p)
...
if __name__ =="__main__":
    ...
    if len(sys.argv)>1:
        ...
        group=15
    print('train group:',group)
    os.system('mkdir model_output_%d'%group)
    lock_dones = threading.Lock()

    open('done.txt', 'a').close()
    done=set([i.strip() for i in open('done.txt') if i.strip() != ''])

    if 'p_dict.json' in os.listdir('./'):
        p_dict=json.load(open('p_dict.json','r'))
    else:
        get_p_dict(table)
    if 'id_group' not in os.listdir('./'):
        split_task()                        #group train task

    trained_num=0
    train_id=json.load(open('id_group/id_group_%d.json'%group,'r'))
    models=[]
    for p_id in train_id:
        if p_id in done:
            continue
        data={}
        data["p_id"]=p_id
        ...
        try:
            ...
            model["profile"] = p
            with open('model_output_%d/%s'%(group,p_id),'wb') as f_model:
                pickle.dump(m,f_model)
            if lock_dones.acquire():
                with open('done.txt','a') as f_done:
                    f_done.write(p_id+'\n')
                lock_dones.release()
            ...
            model["profile"]=None
        models.append(model)
    with open('Models_%s_%d.pickle'%(table,group),'wb') as output:
